...
for svg_file in svg_files:
    ...
    # Build the main script with the correct file path
    t0 = time.time()
    ...
    sql_load = (
        ...
        "'SELECT @xml = CAST(BulkColumn AS XML) FROM OPENROWSET(BULK N''" +
        svg_path.replace("'", "''") +
        "'', SINGLE_BLOB) AS b'; "
    ...
    try:
        ...
        print(f'  Extracted paths OK')
    except Exception as e:
        print(f'  EXTRACT ERROR: {str(e)[:200]}')
        continue
    ...
    sql_geom = (
        ...
        "all_pts AS ("
        "  SELECT layer_id, subpath_id, point_order, x, y FROM parsed "
        "  UNION ALL "
        "  SELECT layer_id, subpath_id, point_order, x, y FROM closing_pts), "
        "rings AS ("
        ...
        "wkt_shapes AS ("
        "  SELECT layer_id, "
        "    CASE WHEN COUNT(*)=1 THEN 'POLYGON(('+MAX(ring_coords)+'))' "
        "    ELSE 'MULTIPOLYGON('+STRING_AGG('('+ring_coords+')',',') "
        "      WITHIN GROUP (ORDER BY subpath_id)+')' END AS wkt "
        "  FROM rings GROUP BY layer_id) "
        "SELECT p.layer_id, p.fill_hex, "
        "  geometry::STGeomFromText(ws.wkt, 0).STAsText() AS wkt_geom "
        "FROM #paths p "
    ...
    try:
        ...
        print(f'  Results ({len(rows)} rows) in {elapsed:.1f}s:')
        for row in rows:
            layer_id = row[0]
            fill_hex = row[1]
            wkt = row[2]
            if wkt and len(wkt) > 120:
                wkt = wkt[:120] + '...'
            print(f'    layer={layer_id:3d}  fill=#{fill_hex or "none":8s}  '
                  f'geom={wkt or "NULL"}')
    except Exception as e:
        print(f'  GEOM ERROR: {str(e)[:200]}')
